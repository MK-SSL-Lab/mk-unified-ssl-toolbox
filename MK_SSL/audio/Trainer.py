        ...
        **kwargs
        ...
        for epoch in range(epochs):
            ...
            if epoch == stage_freeze_epochs:
                self.logger.info(f"[Wav2Vec2-CTC] Unfreezing backbone at epoch {epoch+1}/{epochs}")
            ...
            self.logger.info(f"[Wav2Vec2-CTC Train] Epoch {epoch+1}/{epochs} - "
                            f"avg_loss: {avg_train_loss:.4f} | lr_head={lr_head:.5g} lr_backbone={lr_backbone:.5g}")

            if self.wandb_logger.is_active:
                self.wandb_logger.log({
                    "wav2vec2/train_loss": avg_train_loss,
                    "wav2vec2/epoch": epoch + 1,
                    "wav2vec2/lr_head": lr_head,
                    "wav2vec2/lr_backbone": lr_backbone
                }, step=epoch + 1)
