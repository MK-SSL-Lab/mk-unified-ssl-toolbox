...
class Trainer:
    ...
    def _linear_probe_cola(
        ...
        **kwargs
        ...
        """
        Linear probing for the COLA model.

        Args:
            eval_dataset (Dataset): Supervised dataset with (x, y) pairs.
            num_classes (int): Number of output classes.
            batch_size (int): Evaluation batch size.
            lr (float): Learning rate.
            max_epochs (int): Max number of epochs.
            freeze_backbone (bool): Whether to freeze the backbone during probing.
        """
        ...
        # Define classifier
        classifier = nn.Linear(self.model.feature_size, num_classes).to(self.device)

        # Optimizer on classifier only
        ...
        for epoch in range(max_epochs):
            ...
            print(f"[Epoch {epoch+1}/{max_epochs}] Loss: {total_loss/total:.4f}, Accuracy: {acc:.2f}%")




    def _linear_probe_hubert(
        self,
        eval_dataset: Dataset,
        num_classes: int,
        batch_size: int = 64,
        ...
        **kwargs
        ...
        """
        Linear probing for HuBERT model.
        ...
        Args:
            ...
            freeze_backbone (bool): Whether to freeze the feature extractor and encoder.
        """
        ...
        embed_dim = self.model.config["encoder_embed_dim"]
        ...
        for epoch in range(max_epochs):
            ...
            for waveforms, labels in eval_loader:
                ...
                padding_mask = waveforms == 0  # Assume 0-padding
                with torch.no_grad():
                    feats = self.model.feature_extractor(waveforms).transpose(1, 2)
            ...
            acc = 100.0 * correct / total
            print(f"[HuBERT Epoch {epoch+1}/{max_epochs}] Loss: {total_loss/total:.4f}, Accuracy: {acc:.2f}%")


    def _linear_probe_simclr(
        self,
        eval_dataset: Dataset,
        num_classes: int,
        batch_size: int = 64,
        ...
        **kwargs
        ...
        """
        Linear probing for SimCLR model.
        ...
        classifier = nn.Linear(self.model.backbone.embed_dim, num_classes).to(self.device)
        optimizer = Adam(classifier.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss()
        ...
        for epoch in range(max_epochs):
            ...
            acc = 100.0 * correct / total
            print(f"[SimCLR Epoch {epoch+1}/{max_epochs}] Loss: {total_loss/total:.4f}, Accuracy: {acc:.2f}%")





    def _linear_probe_wav2vec2(
        self,
        eval_dataset: Dataset,
        num_classes: int,
        batch_size: int = 64,
        ...
        **kwargs
        ...
        """
        Linear probing for Wav2Vec2 model (SSL phase only).

        Args:
            eval_dataset (Dataset): Supervised dataset with (x, y) pairs.
            num_classes (int): Output classes for classifier.
            batch_size (int): Batch size.
            lr (float): Learning rate.
            max_epochs (int): Max number of epochs.
            freeze_backbone (bool): Whether to freeze the feature extractor + encoder.
        """
        ...
        if freeze_backbone:
            for param in self.model.feature_extractor.parameters():
                param.requires_grad = False
            for param in self.model.feature_proj.parameters():
                param.requires_grad = False
            for param in self.model.encoder.parameters():
                param.requires_grad = False

        embed_dim = self.model.model_config["encoder_embed_dim"]
        classifier = nn.Linear(embed_dim, num_classes).to(self.device)
        optimizer = Adam(classifier.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss()
        ...
        for epoch in range(max_epochs):
            ...
            for waveforms, labels in eval_loader:
                waveforms = waveforms.to(self.device)
                labels = labels.to(self.device)
                lengths = torch.full((waveforms.size(0),), waveforms.size(1), dtype=torch.long, device=self.device)

                with torch.no_grad():
                    x, lengths = self.model.feature_extractor(waveforms, lengths)
                    x = self.model.feature_proj(x)
            ...
            acc = 100.0 * correct / total
            print(f"[Wav2Vec2 Epoch {epoch+1}/{max_epochs}] Loss: {total_loss/total:.4f}, Accuracy: {acc:.2f}%")
